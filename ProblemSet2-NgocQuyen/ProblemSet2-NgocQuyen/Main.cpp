...
void runP1() {
    ...
    try {
        int lValue = lVector[lArrayLength];  // Out of range access
    ...
    try {
        ...
        lVector.swap(5, 20);  // Out of range access
...
void runP2() {
    ...
    lVector.sort([](int a, int b) { return a < b; }); // Ascending order
...
void runP3() {
    ...
    lVector.sort([](int a, int b) { return a > b; }); // Descending order
    cout << "After sorting:" << endl;
    for (size_t i = 0; i < lVector.size(); i++) {
        cout << lVector[i] << ' ';
    }
